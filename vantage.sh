...
#Requirement: zenity, networkmanager, pulseaudio or pipewire-pulse
...
# Locate the touchpad's kernel input device via sysfs (Wayland/compositor-agnostic).
# Each /sys/class/input/event*/device dir holds both "name" and the "inhibited" toggle.
touchpad_dev=""
for dev in /sys/class/input/event*/device; do
    name="$(cat "$dev/name" 2>/dev/null)"
    case "$name" in
        *[Tt]ouchpad*) touchpad_dev="$dev"; break ;;
    esac
done
...
get_touchpad_status() {
    # inhibited: 1 = device disabled (Off), 0/empty = enabled (On)
    cat "$touchpad_dev/inhibited" 2>/dev/null | awk '{print ($1 == "1") ? "Off" : "On"}'
}
...
main() {
    while :; do
        ...
        test -n "$touchpad_dev" && options+=("Touchpad" "$(get_touchpad_status)")
        ...
        case "$menu" in
            ...
            "Touchpad")
                ...
                case "$submenu" in
                    "$SUBMENU_ON") echo "0" | pkexec tee "$touchpad_dev/inhibited" ;;
                    "$SUBMENU_OFF") echo "1" | pkexec tee "$touchpad_dev/inhibited" ;;
                esac
                ;;
            "WiFi")
